...
export const rewrite = onRequest(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const categoryHint =
        (category ? CATEGORY_GUIDANCE[category] : null) ?? CATEGORY_GUIDANCE.OTHER;
      ...
      const exampleOutput =
        '{"rewrite":"決められます。","reason":"断定を避ける表現を取り除き、敬体で明確に言い切りました。"}';
            ...
            schema: {
              ...
              properties: {
                rewrite: {
                  type: "string",
                  description:
                    "書き換え後の文章。原文より明確で敬体表現に統一された文を 1 つ返す。",
                },
                reason: {
                  type: "string",
      ...
      const rawOutput = (response as unknown as {
        output?: Array<{
          content?: Array<{
            type: string;
            json_value?: unknown;
      ...
      if (firstContent?.type === "json" && firstContent.json_value) {
        parsed = firstContent.json_value as { rewrite?: string; reason?: string };
      } else if (typeof rawOutput.output_text === "string") {
        try {
          parsed = JSON.parse(rawOutput.output_text ?? "");
        } catch (parseError) {
          console.warn(
            "[rewrite] JSON parse failed",
            parseError,
      ...
      const finalRewrite =
        candidateText && candidateText !== normalisedOriginal ? candidateText : null;
