...
前職で 2 年間、医療関係 SEO コンテンツの品質管理に取り組んでいましたが、曖昧表現の添削・フィードバックに多くの時間を取られていました。
「でしょう」「など」「場合がある」といった曖昧表現は便利ですが、多用しすぎると"意味の解像度"が下がり、記事の品質だけでなく、クリニックの信頼までも落としてしまう直接的な原因にもなります。

そこで、「曖昧さ」を曖昧なままにしない & 曖昧な表現が良くない理由をライターさん自身が執筆の段階で気付けて、編集者の添削工数を減らせるようなアプリを作りたいと思っていました。

具体的には、以下のような機能を備えたアプリです。

- 曖昧な箇所が視覚的に見えること（気づける）
- なぜ曖昧なのかが言葉で分かること（納得できる）
- 言い換えのヒントを得られること（時短できる）
- 修正の積み重ねが自分の成長として可視化されること（続けられる）

特に、修正の成長として可視化される機能は、ライターさんのモチベーション向上にも繋がるだけでなく、短期の契約解除を防ぐ効果も期待できます。
また、曖昧な理由を知れることで同じ理由のフィードバックを削減し、編集者とライターさんの関係性悪化を防げます。

書き手は自分の癖を鏡のように知り, 同じ失敗で止まらなくなる。編集者は真に価値のある指摘（構成・論拠・読者体験）に集中できる。読者は、曖昧さに迷わず要点に辿り着ける。
——その三者が同時に報われる状態を、私はこのプロダクトで目指しています。
...
曖昧表現のピックアップ精度次第な気はしますね。
ツールが分散するのもけっこう手間ではあるので、現状の AI でカバーできる範囲なら新たに使われにくいでしょうし。
教育ツールとして、成長度合いを可視化できるとか、副次的な要素はほしいかなと思います。
